#!/usr/bin/env python3
"""
대신증권 리포트 다운로드 URL 검증 + 후처리 스크립트

문제:
  대신증권 PDF URL(http://money2.daishin.com/PDF/Out/...) 은 모두 유효하지만
  wget 다운로드 과정에서 실패하는 케이스가 있음.

전략:
  1. 모든 대신증권 레코드의 PDF URL을 HEAD 검증
  2. 실패 시 HTTPS 변환 + article_url 재방문(btnPdfLoad) 으로 복구 시도
  3. 통계 보고
"""
import asyncio
import aiohttp
import os
import sys
from datetime import datetime
from loguru import logger
from bs4 import BeautifulSoup

sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))
from models.db_factory import get_db


SEC_FIRM_ORDER = 17
FIRM_NAME = "대신증권"

HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36",
    "Referer": "https://money2.daishin.com/",
}
FETCH_TIMEOUT = aiohttp.ClientTimeout(total=30)


async def head_test(session: aiohttp.ClientSession, url: str) -> int | None:
    """HEAD 요청으로 URL 상태 확인"""
    try:
        async with session.head(url, headers=HEADERS, timeout=10, ssl=False) as resp:
            return resp.status
    except Exception:
        return None


async def extract_pdf_from_article(session: aiohttp.ClientSession, article_url: str) -> str | None:
    """article_url 상세 페이지에서 btnPdfLoad href 추출"""
    try:
        async with session.get(article_url, headers=HEADERS, timeout=FETCH_TIMEOUT, ssl=False) as resp:
            if resp.status != 200:
                return None
            html = await resp.text()
            soup = BeautifulSoup(html, "html.parser")
            btn = soup.find(id="btnPdfLoad")
            if btn and btn.get("href"):
                return btn["href"]
    except Exception:
        pass
    return None


async def fix_daeshin_urls():
    db = get_db()

    records = await db.execute_query("""
        SELECT report_id, "telegram_url", "pdf_url", "article_url",
               "article_title", "writer"
        FROM "tbl_sec_reports"
        WHERE "sec_firm_order" = %s
        ORDER BY "report_id" DESC
    """, (SEC_FIRM_ORDER,))

    if not records:
        logger.info(f"[{FIRM_NAME}] 처리할 레코드가 없습니다.")
        return

    total = len(records)
    logger.info(f"[{FIRM_NAME}] 전체: {total}건")

    ok_count = 0
    fail_count = 0
    recovered_https = 0
    recovered_article = 0
    dead_count = 0

    async with aiohttp.ClientSession(
        connector=aiohttp.TCPConnector(ssl=False),
        timeout=FETCH_TIMEOUT,
    ) as session:
        for i, r in enumerate(records, 1):
            report_id = r["report_id"]
            pdf_url = r["pdf_url"] or r["telegram_url"] or ""
            article_url = r.get("article_url") or ""
            title = r.get("article_title", "")[:50]

            if not pdf_url:
                dead_count += 1
                continue

            # 1. HEAD 검증
            status = await head_test(session, pdf_url)

            if status == 200:
                ok_count += 1
                if i % 500 == 0:
                    logger.info(f"[{FIRM_NAME}] 진행 {i}/{total} (OK={ok_count}, FAIL={fail_count})")
                continue

            # 2. 실패 → HTTPS 변환 시도
            logger.warning(f"[{report_id}] HTTP 실패 (status={status}): {title}")
            https_url = pdf_url.replace("http://", "https://")
            https_status = await head_test(session, https_url)

            if https_status == 200:
                await db.update_telegram_url(
                    record_id=report_id,
                    telegram_url=https_url,
                    article_title=r.get("article_title"),
                    pdf_url=https_url,
                )
                logger.success(f"  ✓ HTTPS 복구: {https_url[:90]}")
                recovered_https += 1
                ok_count += 1
                continue

            # 3. HTTPS도 실패 → article_url 재방문
            if article_url:
                new_pdf = await extract_pdf_from_article(session, article_url)
                if new_pdf and new_pdf != pdf_url:
                    new_status = await head_test(session, new_pdf)
                    if new_status == 200:
                        await db.update_telegram_url(
                            record_id=report_id,
                            telegram_url=new_pdf,
                            article_title=r.get("article_title"),
                            pdf_url=new_pdf,
                        )
                        logger.success(f"  ✓ article 재추출 복구: {new_pdf[:90]}")
                        recovered_article += 1
                        ok_count += 1
                        continue

            fail_count += 1
            logger.warning(f"  ✗ 복구 불가: report_id={report_id}")
            await asyncio.sleep(0.1)

    logger.info("")
    logger.info("=" * 60)
    logger.info(f"[{FIRM_NAME}] 검증 완료")
    logger.info(f"  전체:     {total}건")
    logger.info(f"  정상:     {ok_count}건")
    logger.info(f"  실패:     {fail_count}건")
    logger.info(f"  - HTTPS   복구: {recovered_https}건")
    logger.info(f"  - article 재추출: {recovered_article}건")
    logger.info("=" * 60)


if __name__ == "__main__":
    log_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), "logs")
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, "fix_daeshin_db.log")

    logger.remove()
    logger.add(sys.stderr, level="INFO",
               format="<green>{time:HH:mm:ss}</green> | <level>{level:<8}</level> | {message}")
    logger.add(log_file, level="DEBUG",
               rotation="10 MB", retention="14 days", encoding="utf-8",
               format="{time:YYYY-MM-DD HH:mm:ss} | {level:<8} | {message}")

    logger.info(f"[{FIRM_NAME}] PDF URL 검증 시작")
    asyncio.run(fix_daeshin_urls())
    logger.info(f"[{FIRM_NAME}] PDF URL 검증 종료")
